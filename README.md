...
Чтобы улалить триггер, используйте **#unact** или **#ungrep** и полную строчку триггера, отображаемую в #triggers.
...
В отличии от AMC, возможно использовать алиас из нескольких слов, например:
```
...
В отличии от всех остальных видов объектов (триггеры, алиасы и т.п.), переменные существуют в профиле, а не в группе.
...
```
#sub {изначальная строка} {желаемая строка} {приоритет} {группа} -- последние два параметра опциональны
```
Снова, как и прежде, неуказание группы делает сабститут временным.
...
Для использования hex-значений, его следует брать в кавычки и начинать с (#). Пример:
```html
#echo <color fg="#a7ff35">текст</color>
```
...
```
#previewZone название зоны -- открое зону на карте. Выйти из режима просмотра можно сходив на любую соседнюю клетку.
#path название зоны или клетки -- найдет путь и поведет вас в зону или клетку.
```

```
#zones уровень -- напечатает зоны для уровня. Тоже самое, что "зоны" в игре, но в отличии от мадовской команды, может отображать зоны для 0 уровня.
```

## DSL-скрипты
Вместо использования стандартных команд #act и пр., вы можете писать скрипты на Котлине. Это весьма удобно и экспрессивно.
В папке `Documents\Silmaril\dsl` создайте файлик **Misc.mud.kts**, напишите ваш скрипт, перезапустите клиент и не забудьте активировать группу Misc в клиенте – **#group enable misc**.
...
Все из них имеют два варианта второго параметра: ламбду (как на примерах выше) или стринг.
...
Приведу пример скрипта, который получает все квесты в заданном диапазоне уровней, напр. команда "квест 10" получит все квесты 10 уровня, а "квест 10-15" – все квесты от 10 до 15.
```kts
/** Quests */
...
// Прекращаем собирать квесты либо при получении данной строки, либо через 2.5 сек после начала (т.к. эта строка не всегда приходит)
