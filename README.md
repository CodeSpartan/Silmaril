...
> [!NOTE]
> На Android в режиме File Transfer, путь к лорам: `Android/data/ru.adan.silmaril/files/Silmaril/lores`
> 
> На Android, посещенные клетки можно перенести только из Silmaril на ПК, после того, как они были импортированы туда из AMC. Скопируйте содержание папки `~/Documents/Silmaril/Maps/` в папку `Android/data/ru.adan.silmaril/files/Silmaril/maps/` 
...
Клиент поддерживает вычисление математических выражений и условных конструкций прямо в командах.
...
```
#echo $math(2 + 3)                    → 5
#echo $math(10 / 3)                   → 3.333...
```

Возможны вложенные выражения:
```
#echo $math($math(2+3) * $math(4+1))  → 25
```
...
Поддерживаемые операторы: `+`, `-`, `*`, `/`, `%` (остаток), `^` (степень)

#### $if()()()

Условная конструкция с тремя блоками: условие, значение если истина, значение если ложь.

Синтаксис: `$if(условие)(значение_если_да)(значение_если_нет)`

Примеры с числовыми сравнениями:
```
#var {hp} {30}
#echo $if($hp < 50)(Мало HP!)(HP в норме)     → Мало HP!
#echo $if($hp == 30)(точно 30)(не 30)         → точно 30
#echo $if($hp != 100)(не полное)(полное)      → не полное
#echo $if($hp >= 20)(достаточно)(мало)        → достаточно
```

Строковые сравнения (in-line строки в кавычках):
```
#var {target} {орк}
#var {target2} {орк}
#echo $if($target == $target2)(да)(нет)           → да
#echo $if($target == "орк")(это орк)(не орк)      → это орк
#echo $if($target != "гоблин")(не гоблин)(гоблин) → не гоблин
```

Комбинация с командами:
```
#var {hp} {30}
$if($hp < 50)(#echo Внимание! Мало HP!)(#echo HP в порядке)
```

С несколькими командами через точку с запятой:
```
$if($hp < 50)(#echo LOW;#var {status} {danger})(#echo OK)
```
При hp < 50 выполнятся обе команды: `#echo LOW` и `#var {status} {danger}`

Использование в триггерах:
```
#act {У вас %1 очков здоровья} {#var {hp} {%1};$if($math(%1 < 50))(#echo Мало HP!)(#echo HP OK)}
```

Поддерживаемые операторы сравнения: `==`, `!=`, `<`, `>`, `<=`, `>=`
<br>Логические операторы: `&&` (и), `||` (или), `!` (не)
...
Все возможные значения перечислены в файле [HotkeyEvent.kt](src/jvmMain/kotlin/ru/adan/silmaril/misc/HotkeyEvent.kt#L51-L76)
...
Таким образом, DSL-скрипты дают полную свободу в написании сложной логики на Kotlin.
...
===============================================================================
 Kotlin                 66        16327        13403         1133         1791
```