...
import ru.adan.silmaril.platform.setMdcProfile
import ru.adan.silmaril.platform.clearMdcProfile
import ru.adan.silmaril.platform.MccpInflater
...
class MudConnection(
...
) : GroupDataSource, MobDataSource {
    private val logger = createLogger("ru.adan.silmaril.model.MudConnection")
    private val gameEventsLogger = createLogger("GameEvents")
    ...
    // Flow to emit received text messages to whoever is listening (MainViewModel and MapViewModel in this case)
    ...
    override val unformattedTextMessages = _unformattedTextMessages.asSharedFlow()
    ...
    override val lastGroupMessage: StateFlow<List<Creature>> get() = _lastGroupMessage

    private val _lastMonstersMessage = MutableStateFlow(RoomMobs.EMPTY)
    override val lastMonstersMessage: StateFlow<RoomMobs> get() = _lastMonstersMessage
    ...
    private var inflater: MccpInflater? = null
    ...
    suspend fun performConnect(): Boolean {
        return try {
            logger.info { "[$profileName] Connecting to $host:$port" }
            ...
            logger.info { "[$profileName] Connection established to $host:$port" }
            ...
            logger.warn { "[$profileName] Connection failed to $host:$port: ${e.message}" }
            cleanupOnDisconnect()
            false
        }
    }
    ...
    private fun cleanupOnDisconnect() {
        try {
            ...
            logger.warn { "Error while closing connection: ${e.message}" }
        } finally {
            stopDecompression()
            socket = null
        ...
        readJob = connectionScope.launch {
            try {
                ...
                while (isActive) {
                    ...
                    if (host == "adan.ru" || host == "138.201.133.21" || host == "localhost") {
    ...
    private fun decompress(data: ByteArray): ByteArray {
        ...
        val result = inf.decompress(data)

        if (inf.isStreamEnded()) {
            logger.info { "[inflate] Z_STREAM_END reached. Stopping decompression." }
            stopDecompression()
            connectionScope.launch {
    ...
    private fun bufferToColorfulText(buffer: ByteArray, bufferEndPointer: Int) : ColorfulTextMessage {
        ...
        // sometimes messages start with \r, in that case just skip the first character
        val startAtIndex = if (buffer[0] == ControlCharacters.CarriageReturn) 1 else 0

        var skipNextByte = false
        for (offset in startAtIndex until bufferEndPointer) {
            if (skipNextByte) {
                skipNextByte = false
                continue
            }
        ...
        if (colorTreatmentPointer > 0) {
            ...
            // sometimes the text starts with \r or \r\r, which we need to remove
    ...
    private suspend fun processTextMessage(buffer: ByteArray, bufferEndPointer: Int, emitEmpty: Boolean) {
        ...
        if (gluedMessage.chunks.isNotEmpty()) {
            ...
            // send glued string to the logger with MDC context for per-profile log files
            setMdcProfile(profileName)
            try {
    ...
    private fun emptyTextMessage() =
        ColorfulTextMessage(arrayOf(TextMessageChunk("", AnsiColor.None)))
