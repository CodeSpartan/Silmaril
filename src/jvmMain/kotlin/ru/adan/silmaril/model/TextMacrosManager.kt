...
class TextMacrosManager() : KoinComponent {
    ...
    private val logger = createLogger("ru.adan.silmaril.model.TextMacrosManager")
    ...
    @OptIn(ExperimentalAtomicApi::class)
    public suspend fun initExplicit(callerProfile: ProfileInterface) {
    ...
    private suspend fun loadTextTriggers() : Map<String, List<SimpleTriggerData>> {
        return withContext(Dispatchers.IO) {
            ...
            triggerFiles.associate { triggerFile ->
                ...
                val yamlToParse = yaml.ifEmpty { "[]" }   // empty file => empty list
                val triggers = Yaml.default.decodeFromString<List<SimpleTriggerData>>(string = yamlToParse)
    ...
    private suspend fun loadTextAliases() : Map<String, List<SimpleAliasData>> {
        return withContext(Dispatchers.IO) {
            ...
            aliasFiles.associate { aliasFile ->
                ...
                val yamlToParse = yaml.ifEmpty { "[]" }   // empty file => empty list
                val aliases = Yaml.default.decodeFromString<List<SimpleAliasData>>(string = yamlToParse)
    ...
    private suspend fun loadTextSubs() : Map<String, List<SimpleTriggerData>> {
        return withContext(Dispatchers.IO) {
            ...
            subsFiles.associate { subFile ->
                ...
                val yamlToParse = yaml.ifEmpty { "[]" }   // empty file => empty list
                val subs = Yaml.default.decodeFromString<List<SimpleTriggerData>>(string = yamlToParse)
                groupName to subs
            }
        }
    }
    ...
    private suspend fun loadHotkeys() : Map<String, List<SimpleHotkeyData>> {
        return withContext(Dispatchers.IO) {
            ...
            hotkeyFiles.associate { hotkeyFile ->
                ...
                val yamlToParse = yaml.ifEmpty { "[]" }   // empty file => empty list
                val hotkeys = Yaml.default.decodeFromString<List<SimpleHotkeyData>>(string = yamlToParse)
