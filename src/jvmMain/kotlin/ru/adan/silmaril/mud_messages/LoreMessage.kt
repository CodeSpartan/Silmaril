...
data class LoreMessage(
    ...
    companion object {
        private val logger = createLogger("ru.adan.silmaril.mud_messages.LoreMessage")
    ...
    fun printWeaponStats(): Array<String> =
        weaponStats?.let { ws ->
            buildList {
                add("Сила удара '${ws.diceSides}D${ws.diceCount}', средняя сила удара в раунд ${ws.averageDamage}.")
                if (ws.requiredSkill.isNotBlank()) {
                    add("Требует знаний в области '${ws.requiredSkill}'")
                }
            }.toTypedArray()
        } ?: emptyArray()
        ...
        return if (wear == null || wear.wearSlots.isEmpty()) {
            ...
            for (wearSlot in wear.wearSlots) {
                val wearLine = when (wearSlot) {
                    ...
                    "EARS" -> "Наверное, вы сможете продеть это в уши."
                    "EYES" -> "Наверное, вы сможете надвинуть это на глаза."
                    ...
                    "SHIELD" -> "Наверное, вы сможете использовать это как щит."
