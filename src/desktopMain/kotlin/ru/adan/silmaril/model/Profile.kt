...
import ru.adan.silmaril.scripting.addHotkeyToGroup
import ru.adan.silmaril.scripting.getGroupAffects
import ru.adan.silmaril.scripting.getMobsAffects
...
class Profile(
    ...
    override val client: MudConnection by lazy {
        get {
            parametersOf(
                if (getVariable("serverHost") != null) getVariable("serverHost").toString() else settingsManager.settings.value.gameServer,
    ...
    fun onSystemMessage(messageUntrimmed: String, recursionLevel: Int = 0) {
        ...
        when (messageUntrimmed.trim().substringBefore(" ")) {
            ...
            "#moveWindow" -> parseMoveWindowCommand(message)
    ...
    fun onInsertVariables(message: String) : String {
        ...
        val afterMath = evaluateMathExpressions(message) { varName ->
            when (varName) {
                "time" -> currentTime()
                else -> profileData.value.variables[varName]?.toString()
            }
        }

        // Then replace remaining $variables
        return afterMath.replace(insertVarRegex) { matchResult ->
    ...
    fun addSingleTriggerToAll(condition: String, action: String, groupName: String, priority: Int, isRegex: Boolean) {
        ...
        profileManager.getAllOpenedProfiles().forEach { profile -> profile.scriptingEngine.addTriggerToGroup(groupName, newTrigger) }
    }
    ...
    fun addSingleAliasToAll(shorthand: String, action: String, groupName: String, priority: Int) {
        ...
        profileManager.getAllOpenedProfiles().forEach { profile -> profile.scriptingEngine.addAliasToGroup(groupName, newAlias) }
    }
    ...
    fun addSingleSubToAll(condition: String, action: String, groupName: String, priority: Int, isRegex: Boolean) {
        ...
        profileManager.getAllOpenedProfiles().forEach { profile -> profile.scriptingEngine.addSubstituteToGroup(groupName, newSub) }
    }
    ...
    fun addSingleHotkeyToAll(hotkey: String, action: String, groupName: String, priority: Int) {
        ...
        if (newHotkey != null)
            profileManager.getAllOpenedProfiles().forEach { profile -> profile.scriptingEngine.addHotkeyToGroup(groupName, newHotkey) }
    }

    override fun addSingleTriggerToWindow(condition: String, action: String, groupName: String, priority: Int, isRegex: Boolean) {
        val newTrigger =
            if (isRegex) Trigger.regCreate(condition, action, priority, false)
            else Trigger.create(condition, action, priority, false)
    ...
    override fun addSingleAliasToWindow(shorthand: String, action: String, groupName: String, priority: Int) {
        val newAlias = Trigger.createAlias(shorthand, action, priority, false)
        scriptingEngine.addAliasToGroup(groupName, newAlias)
    }

    override fun addSingleSubToWindow(shorthand: String, action: String, groupName: String, priority: Int, isRegex: Boolean) {
        val newSub =
    ...
    override fun addSingleHotkeyToWindow(keyString: String, action: String, groupName: String, priority: Int) {
    ...
    fun parseTextTrigger(message: String) {
        ...
        if (groupName == "SESSION") {
            ...
            profileManager.getAllOpenedProfiles().forEach { profile ->
    ...
    fun parseMoveWindowCommand(message: String) {
        // matches #moveWindow {id}, starting with 1
        val moveWindowIdRegex = """\#moveWindow [{]?(\d+)[}]?$""".toRegex()
        val match = moveWindowIdRegex.find(message)
        if (match == null) {
            mainViewModel.displayErrorMessage("Ошибка #moveWindow - не смог распарсить. Правильный синтаксис: #windowId {номер окна начиная с 1}.")
            return
        }
        val desiredWindowPosition = match.groupValues[1].toInt()-1
        profileManager.moveGameWindow(profileName, desiredWindowPosition)
    }
