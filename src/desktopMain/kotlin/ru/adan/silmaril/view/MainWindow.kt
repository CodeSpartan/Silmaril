...
fun HistoryTextField(
    ...
    fun commit(shiftPressed: Boolean = false) {
        ...
        //logger.info { "Commited \"${inputTextField.text}\" with shift: $shiftPressed" }
        mainViewModel.treatUserInput(if (shiftPressed) "о;вст;${inputTextField.text}" else inputTextField.text)
        historyIndex = -1
    }
    ...
    BasicTextField(
            ...
            .onPreviewKeyEvent { e ->
                ...
                when (e.key) {
                    Key.Enter, Key.NumPadEnter -> {
                        ...
                        commit(shiftPressed = shift)
                        true // consume so no newline, etc.
                    }
        ...
        singleLine = true,
        // keep these for potential mobile targets, but Desktop commit is handled through onPreviewKeyEvent above.
        keyboardOptions = KeyboardOptions.Default.copy(imeAction = ImeAction.Done),
        keyboardActions = KeyboardActions(
            onDone = {
                commit(shiftPressed = false)
            }
        ),
